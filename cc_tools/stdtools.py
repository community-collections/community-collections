...
str_types = (str, unicode) if sys.version_info < (3, 0) else (str,)  # noqa: F821,E501
basestring = string_types = str_types = (str, unicode) if sys.version_info < (3, 0) else (str,)  # noqa: F821,E501
...
def say(text, *flags):
    """Colorize the text."""
    # three-digit codes: first one is style (0 and 2 are regular, 3 is italics,
    # 1 is bold)
    colors = {'gray': (0, 37, 48), 'cyan_black': (1, 36, 40),
              'red_black': (1, 31, 40), 'black_gray': (0, 37, 40),
              'white_black': (1, 37, 40), 'mag_gray': (0, 35, 47)}
    # no colors if we are logging to a text file because nobody wants all that
    # unicode in a log
    if flags and hasattr(sys.stdout, 'isatty') and sys.stdout.isatty():
        ...
        for f in flags[::-1]:
            ...
            text = '\x1b[%sm%s\x1b[0m' % \
                (';'.join([str(style), str(fg), str(bg)]), text)
    return text
...
def color_printer(override=False, prefix=None):
    ...
    if builtins.__dict__['print'].__name__ != 'print_stylized':
        # every script must import print_function from __future__ or syntax
        # error hold the standard print
        _print = print
        key_leads = ['status', 'warning', 'error', 'note', 'usage',
                     'exception', 'except', 'question', 'run', 'tail',
                     'watch', 'bash', 'debug']
        ...
        def print_stylized(*args, **kwargs):
            ...
            # note that we can retire all print('debug','message') statements
                ...
                if match:
                    return _print(prefix+'[%s]' % match.group(1).upper() +
...
def bash(command, log=None, cwd=None, inpipe=None, scroll=True, tag=None,
    ...
    """
    Run a bash command.
    Development note: tee functionality would be useful however you cannot use
    pipes with subprocess here.  Vital note: log is relative to the current
    location and not the cwd.
    """
    if announce:
        print('status', 'ortho.bash%s runs command: %s' % (' (at %s)' % cwd
              if cwd else '', str(command)))
        ...
        if local:
            if log:
                log = os.path.relpath(log, cwd_local)
            pwd = os.getcwd()
            os.chdir(cwd_local)
    if log is None:
        ...
        if inpipe and scroll:
            raise Exception('cannot use inpipe with scrolling output')
        if inpipe:
        ...
        # no log and no input pipe
            ...
            # no scroll waits for output and then checks it below
                ...
                stdout, stderr = proc.communicate()
    # alternative scroll method via
    # https://stackoverflow.com/questions/18421757 special scroll is useful for
    # some cases where buffered output was necessary
    # this method can handle universal newlines while the threading method
    # cannot
        ...
        with open(log, 'ab') as fp:
            for _ in range(2):
                for _, line in iter(qu.get, None):
                    ...
                    #   which makes the output appear inconsistent (some lines
                    #   are prefixed) so we replace newlines, but only if we
                    #   are also reporting the log file on the line next to the
                    #   output. this can get cluttered so you can turn off
                    #   scroll_log if you want
                    if scroll_log:
                        line = re.sub('\r\n?', r'\n', line_decode)
                    ...
                    # note that the encode/decode events in this loop work for
                    #     ascii and unicode in both python 2 and 3, however
                    #     python 2 (where we recommend importing
                    #     unicode_literals) will behave weird if you print from
                    #     a script called through ortho.bash due to locale
                    #     issues described here:
                    #     https://pythonhosted.org/kitchen/unicode-frustrations.html
                    #     so just port your unicode-printing python 2 code or
                    #     use a codecs.getwriter
                    print(line_here, end='')
    ...
    # log to file and suppress output
        ...
        if not inpipe:
            ...
            stdout, stderr = proc.communicate(input=inpipe.encode('utf-8'))
    else:
        raise Exception('invalid options')
    if not scroll and stderr:
        if stdout and not quiet:
            print('error', 'stdout: %s' % stdout.decode('utf-8').strip('\n'))
        if stderr and not quiet:
            print('error', 'stderr: %s' % stderr.decode('utf-8').strip('\n'))
    ...
    if proc.returncode and exit_error:
        if log:
            raise Exception('bash error, see %s' % log)
        else:
            if stdout and not quiet:
                print('error', 'stdout:')
            ...
            raise Exception(
                'bash error with returncode %d and stdout/stderr printed above'
                % proc.returncode)
    if scroll:
        proc.stdout.close()
        if not merge_stdout_stderr:
            proc.stderr.close()
    if local:
        os.chdir(pwd)
    if not scroll:
...
def introspect_function(func, **kwargs):
    ...
    if sys.version_info < (3, 3):
        ...
        if defaults:
            ...
            packed = dict(args=tuple(args), kwargs={})
        return packed
    else:
...
def asciitree(obj, depth=0, wide=2, last=[], recursed=False):
    ...
    if type(obj) in [float, int, bool]+str_types_list:
        if depth == 0:
            ...
            all([type(i) in [str, float, int, bool] for i in obj.values()]) \
            and depth == 0:
        ...
        for ind, item in enumerate(obj):
            ...
            if type(item) in [float, int, bool] + str_types_list:
                print(spacer_this+str(item))
            elif item != {}:
                ...
                print('unhandled tree object %s' % item)
    elif isinstance(obj, dict) and obj != {}:
        for ind, key in enumerate(obj.keys()):
            spacer_this = spacer_both['end'] if ind == len(obj)-1 else spacer
            if type(obj[key]) in [float, int, bool]+str_types_list:
                print(spacer_this+str(key)+' = '+str(obj[key]))
            # special: print single-item lists of strings on the same line as
            # the key
            elif type(obj[key]) == list and len(obj[key]) == 1 and \
                    type(obj[key][0]) in [str, float, int, bool]:
            ...
            # special: skip lists if blank dictionaries
                ...
                print(spacer_this+'%s = {}' % key)
            else:
                print('unhandled tree object %s' % key)
    else:
        print('unhandled tree object %s' % obj)
    if not recursed:
        print('\n')
...
def treeview(data, style='unicode'):
    ...
    """
    if style == 'unicode':
        # protect against TeeMultiplexer here because it cannot print unicode
        # to the log file
        ...
        raise Exception('invalid style %s' % style)
...
class Handler(object):
    ...
    def _report(self):
        ...
        if not self._taxonomy:
            print('debug There are no methods.')
        else:
    ...
    def _classify(self, *args):
        ...
        if len(matches) == 0:
            if not self.lax:
                ...
                spills = [(i, set.difference(set(args),
                           set.union(j['base'],
                           j['opts'])))
                          for i, j in self._taxonomy.items()
                          if i in spillovers]
                if not spills:
                    self._matchless(args)
                ...
                if len(matches_lax) == 0:
                    self._matchless(args)
                elif len(matches_lax) == 1:
                    return matches_lax[0]
                else:
                    ...
                    self._report()
                    raise Exception(
                        'In lax mode we have redundant matches.'
                        ' Your arguments (%s) are equally compatible with'
                        ' these methods: %s' % (list(args), matches_lax))
        elif len(matches) > 1:
    ...
    def _taxonomy_inference(self):
        ...
        """
        # note that all functions that start with "_" are invalid target
        # methods
        ...
        if len(defaults) > 1:
            raise Exception(
                'More than one function accepts only **kwargs: %s' % defaults)
        elif len(defaults) == 1:
        ...
        for key in self._taxonomy:
            ...
            collide = [i for i in self._internals.values() if i in argnames]
            if any(collide):
                collisions[key] = collide
        if any(collisions):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(self, fname):
            raise Exception(
                'development error: taxonomy name "%s" is not a member' %
                fname)
...
def tracebacker(*args, **kwargs):
    ...
    # note: previously handled interrupt here but this prevents normal
    # traceback
    if len(args) == 1 or len(args) == 0:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        tracebacker_base(exc_type, exc_obj, exc_tb, debug=debug)
    elif len(args) == 3:
        tracebacker_base(*args, debug=debug)
    else:
        raise Exception(
...
def confirm(*msgs, **kwargs):
    ...
    return sure or all(
        re.match('^(y|Y)', (
            input if sys.version_info > (3, 0) else raw_input)
            ('[QUESTION] %s (y/N)? ' % msg)) is not None for msg in msgs)
