...
class UseCase(Handler):
    ...
    def _stage_bashrc_changes(self):
        ...
        if mods:
            ...
            mods_prev = \
                self.cache['settings'].get('profile', {}).get('mods', [])
            mods = mods_prev + mods
    ...
    def main(self,
        ...
        try:
            lmod_inst = Convey(
                cache=self.cache,
                _register_error=register_error)(LmodManager)(**lmod)
        # defer exceptions
        ...
        try:
            singularity_inst = Convey(cache=self.cache,
        ...
        if spack:
            try:
                spack_inst = Convey(cache=self.cache,
                                    _register_error=register_error
                                    )(SpackManager)(**spack)  # noqa
            # null value since this is optional
            except Exception as e: spack_inst = False  # noqa
        ...
        for name, error in errors.items():
            ...
            # if not a dict we send a string to explain the error
                ...
                print('status received error: %s' % error)

        # exit on error
        if errors:
            self._shutdown()
            # exceptions are too verbose so we tell user to edit and exit
            print(say('[CC]', 'mag_gray') + ' ' +
                  say('[STATUS]', 'red_black') +
        ...
        result = bash(os.path.join(
                      singularity_inst.path,
                      singularity_inst.check_bin_version),
                      scroll=False)
        try:
            version = re.match(r'(?:^|.+\s)(\d+\.\d+(?:\.\d+))',
                               result['stdout']).group(1)
        except:  # noqa - no use of bare 'except'?
            raise Exception('failed to infer Singularity version')
...
class VersionCheck(Handler):
    ...
    def _check_version(self, splits, target, prefer_no_suffix=True):
        ...
        for split in splits:
            ...
            elif not suffix and (re.match(r'^[\d\.]+$', this)
                                 and self._version_check(this, op, version)):
...
class ModuleRequest(Handler):
    ...
    def singularity_pull(self, name, source=None,
        ...
        if not source:
            source = self.cache['module_settings']['source']
        ...
        if source == 'docker':
            ...
            raise Exception('UNDER DEVELOPMENT, source: %s' % source)
        ...
        if versions:
            if detail.get('extras', None):
                ...
                detail['extras'] = ''
            self._write_modulefile(dn=dn, fn='.base', text=text % detail)
        ...
        for tag in versions:
            ...
            if not os.path.isfile(target_link):
                os.symlink(os.path.join('.base.lua',), target_link)
